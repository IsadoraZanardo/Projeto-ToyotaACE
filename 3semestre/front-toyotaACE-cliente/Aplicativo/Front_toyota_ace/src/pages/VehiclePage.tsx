...
const baseSteps = [
  { label: "Pedido Realizado", color: "bg-blue-500", description: "Seu pedido foi registrado no sistema da concessionária." },
  { label: "Linha de Produção", color: "bg-yellow-500", description: "O veículo entrou na linha de produção da fábrica." },
  { label: "Inspeção", color: "bg-orange-500", description: "São realizados testes de qualidade e segurança." },
  { label: "Cegonha", color: "bg-red-500", description: "Seu veículo está em transporte, a caminho da concessionária." },
  { label: "Concessionária", color: "bg-pink-500", description: "Seu veículo chegou na concessionária." },
  { label: "Pronto para Retirada", color: "bg-green-500", description: "Seu veículo está pronto para ser retirado." },
];
...
const VehiclePage = () => {
  ...
  const [iotSteps, setIotSteps] = useState<string[]>([]);
  ...
  try {
    const resposta = await fetch("http://localhost:8083/iot/status/ABC123");

    const dados = await resposta.json();

    console.log("DADOS IOT:", dados);

    setIotSteps(dados);
  } catch (err) {
    console.error("Erro IoT:", err);
  }
}
  ...
  const steps = useMemo(
    () =>
      baseSteps.map((step) => ({
        ...step,
        done: iotSteps.includes(step.label),
      })),
    [iotSteps]
  );
            ...
            <Card className="bg-white dark:bg-zinc-900 border shadow-sm">
              <CardHeader>
                <CardTitle className="text-xl">
                  {vehicleData?.modeloVeiculo || "Veículo Toyota"}
                </CardTitle>
              </CardHeader>
